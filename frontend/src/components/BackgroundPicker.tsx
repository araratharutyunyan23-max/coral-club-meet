...
/**
 * The background grid: None + blur + image presets (one radiogroup + the
 * "unavailable" note). Reused by the lobby picker popover and the in-call effects
 * panel — the chrome (trigger button / popover / sheet) lives in the callers.
 * Styles: .bgp-*.
 */
export function BackgroundGrid({
  ...
  const t = useT()
  const blurs = BACKGROUNDS.filter((b) => b.kind === 'blur')
  ...
  // Blur, as a grid tile (needs the segmenter, so disabled when effects are off).
  const BlurTile = (p: BgPreset) => {
    const on = value === p.id
    return (
      <button
        key={p.id}
